...
# hyper param
batch_size = 64  # how many independant sequence will be process in parallel
block_size = 256  # what is the max context len for prodeiction
max_iters = 5000
eval_interval = 500
learning_rate = 3e-4
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        out = wei @ v
        return out  # new x embedding of what the token means in this context
...
class FeedForward(nn.Module):
    ...
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_embd, 4 * n_embd),  # what the
            nn.ReLU(),
            nn.Linear(4 * n_embd, n_embd),  # fuck
            nn.Dropout(dropout),
        )
...
class Block(nn.Module):
    # Transformer block: communication by computation

    def __init__(self, n_embd, n_head):
        # n_embd embedding dimension, n_head: nb of head we d like to use
        super().__init__()
...
class BigramLanguageModel(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.Sequential(
            *[Block(n_embd, n_head=n_heads) for _ in range(n_layers)]
        )
